...
int main(int argc, char *argv[]) {
	...
	if (http_code_num == 200) {
		...
		if (file_name != NULL) {
			...
			FILE *bfp = fopen(file_name, "wb");
			//write what is inside the buffer minus the header
			fwrite(data_here, sizeof(char), data_here_size, bfp);
			...
			//close file
                        fclose(bfp);
                        printf("File successfully downloaded!! Yea!! :)\n");
		}
